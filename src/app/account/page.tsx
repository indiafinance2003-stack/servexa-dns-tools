...
export default async function AccountPage(): Promise<React.ReactElement> {
  ...
  try {
    saved = await listSavedAnalyses(user.id);
  } catch {
    savedError =
    ...
    <div className="mx-auto max-w-3xl px-4 py-12">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <form method="post" action="/logout">
          <button
            type="submit"
            className="rounded-md border border-line px-4 py-2 text-sm font-medium text-slate-700 hover:border-red-300 hover:text-red-700"
          >
            Log out
          </button>
        </form>
      ...
      <section className="mt-8 rounded-xl border border-line bg-white p-6">
        ...
        <dl className="mt-4 grid gap-x-8 gap-y-3 sm:grid-cols-2">
          ...
          <div>
            <dt className="text-sm text-slate-600">Account created</dt>
            <dd className="mt-0.5 text-sm font-medium text-ink">{formatDate(user.createdAt)} UTC</dd>
          </div>
          <div>
            <dt className="text-sm text-slate-600">Last sign-in</dt>
      ...
      <section className="mt-6 rounded-xl border border-line bg-white p-6">
        ...
        <div className="mt-4">
          {savedError ? (
            <p className="rounded-md bg-amber-50 p-3 text-sm text-amber-900">{savedError}</p>
          ) : (
            <SavedAnalysesList
      ...
      <section className="mt-6 rounded-xl border border-line bg-white p-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm">
          <Link href="/dns/lookup" className="rounded-md border border-line px-3 py-1.5 text-slate-700 hover:border-accent hover:text-accent">
            DNS Lookup
          </Link>
          <Link href="/dns/analyze" className="rounded-md border border-line px-3 py-1.5 text-slate-700 hover:border-accent hover:text-accent">
            DNS Health
          </Link>
          <Link href="/email/analyze" className="rounded-md border border-line px-3 py-1.5 text-slate-700 hover:border-accent hover:text-accent">
            Email Headers
          </Link>
        </div>
      </section>

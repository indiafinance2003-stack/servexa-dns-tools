      ...
      <section className="text-center">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          ...
          <Link
            ...
            className="rounded-md border border-line px-5 py-2.5 text-sm font-medium text-slate-700 hover:border-accent hover:text-accent"
          >
      ...
      <section className="mt-14">
        ...
        <ul className="mt-6 flex flex-wrap justify-center gap-2">
          {TALENT_FOCUS_ROLES.map((role) => (
            <li
              key={role}
              className="rounded-full border border-line bg-white px-4 py-1.5 text-sm text-slate-700"
            >
              {role}
            </li>
          ))}
        ...
        <div className="mt-6">
          ...
          <ul className="mt-2 flex flex-wrap gap-2">
            {TALENT_CLIENT_SEGMENTS.map((segment) => (
              <li
                key={segment}
                className="rounded-full bg-paper px-3 py-1 text-sm text-slate-600"
              >
                {segment}
              </li>
            ))}

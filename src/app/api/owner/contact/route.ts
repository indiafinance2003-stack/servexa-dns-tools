...
/** GET /api/owner/contact — Owner-only contact submission inbox (status filter opt-in). */
export async function GET(req: NextRequest): Promise<Response> {
  return handleApi(req, async () => {
    ...
    return listAllContactSubmissions({ status, limit: Number.isFinite(limit) ? limit : 50 });
  });
}